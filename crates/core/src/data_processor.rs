...
impl DataProcessor {
    ...
    async fn set_data(&self, task: &DataTask) {
        let db_key =
            norn_common::utils::db_keys::data_address_key_to_db_key(&task.address, &task.key);
    ...
    async fn append_data(&self, task: &DataTask) {
        ...
        if let Err(e) = self.db.insert(&db_key, &new_val_bytes).await {
            ...
            info!(
                ...
                hex::encode(&db_key),
                String::from_utf8_lossy(&new_val_bytes)
            );
        }
    ...
    fn emit_event(&self, task: &DataTask, value_str: String) {
        ...
        params.insert(
            "key".to_string(),
            String::from_utf8_lossy(&task.key).to_string(),
        );
