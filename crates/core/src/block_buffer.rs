...
use norn_common::types::{Block, GeneralParams, Hash};
...
impl BlockBuffer {
    ...
    async fn handle_block(&self, block: Block, from_second: bool) {
        ...
        let prev_height_block_hash = state
            .selected_block
            .get(&(height - 1))
        ...
        if !verify_block_vdf(&block, self.vdf_calculator.as_ref()).await {
            warn!(
                "Block VDF verification failed: {} at height {}",
                block_hash, height
            );
            return;
        }
...
async fn verify_block_vdf(
    _block: &Block,
    _vdf_calculator: Option<&Arc<dyn VDFCalculator>>,
) -> bool {
    true
}
...
mod tests {
    ...
    async fn test_buffer_simple_flow() {
        ...
        assert_eq!(
            state.selected_block.get(&1).unwrap().header.block_hash,
