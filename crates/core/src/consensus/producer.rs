...
use anyhow::Result;
use norn_crypto::vrf::{VRFKeyPair, VRFCalculator, VRFOutput};

use crate::blockchain::Blockchain;
use crate::txpool::TxPool;
use crate::merkle::build_merkle_tree;
use crate::consensus::povf::PoVFConfig;
...
impl BlockProducer {
    ...
    async fn check_vrf_selection(&self) -> bool {
        ...
        match VRFCalculator::calculate(&self.vrf_key_pair, &message) {
            Ok(output) => {
                ...
                let threshold = 128u8; // ~50% chance for testing
    ...
    pub async fn produce_block(&self) -> Result<Block> {
        ...
        let mut message = prev_hash.0.to_vec();
        message.extend_from_slice(&(new_height as u64).to_le_bytes());
            ...
            if self.should_produce().await {
                match self.produce_block().await {
                    Ok(block) => {
                        ...
                        // In a full implementation, this would broadcast the block
                        // and add it to the blockchain after consensus
                        
                        // For now, add directly to blockchain
                        if let Err(e) = self.blockchain.save_block(&block).await {
...
mod tests {
    ...
    async fn test_block_producer_creation() {
        ...
        let producer = BlockProducer::new(config, blockchain, tx_pool, vrf_key_pair);
    ...
    async fn test_block_production() {
        ...
        let producer = BlockProducer::new(config, blockchain.clone(), tx_pool, vrf_key_pair);
