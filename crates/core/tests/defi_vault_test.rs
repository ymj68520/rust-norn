...
use norn_core::state::account::{AccountStateConfig, AccountStateManager};
...
async fn test_defi_vault_solidity_contract_execution() {
    ...
    let bin_path = if Path::new("tests/contracts/build/DeFiVault.bin").exists() {
        ...
        Path::new("d:/Programs/blockchain/rust-norn/tests/contracts/build/DeFiVault.bin")
            .to_path_buf()
    };
    let hex_bytecode = fs::read_to_string(bin_path).expect(
        "DeFiVault.bin artifact missing. Ensure solc compiled tests/contracts/DeFiVault.sol",
    );
    let init_bytecode =
        hex::decode(hex_bytecode.trim()).expect("Failed to hex-decode contract bytecode");

    println!(
        "  📄 Loaded DeFiVault Init Bytecode: {} bytes",
        init_bytecode.len()
    );
    ...
    state_manager
        .update_balance(&deployer, initial_balance)
        .await
        .unwrap();
    ...
    let (contract_address, deploy_result) = executor
        .create_contract(
            deployer,
            0, // nonce = 0
            deployment_data,
        ...
        .await
        .expect("DeFiVault Contract deployment failed");

    assert!(
        deploy_result.success,
        "Contract deployment execution reported failure"
    );

    println!(
        "  ✅ DeFiVault Deployed Successfully to Address: {:?}",
        contract_address
    );
    ...
    let add_liq_result = executor
        ...
        .await
        .expect("addLiquidity call execution failed");

    assert!(add_liq_result.success, "addLiquidity failed");
    println!(
        "  ✅ Liquidity Added: Reserve A = 100,000, Reserve B = 200,000 (Gas: {})",
        add_liq_result.gas_used
    );
    ...
    let swap_result = executor
        ...
        .await
        .expect("swap call execution failed");

    assert!(swap_result.success, "AMM Swap execution failed");
    println!(
        "  ✅ Constant-Product AMM Swap Executed (1,000 Token A -> Token B) (Gas: {})",
    ...
    let stake_result = executor
        ...
        .await
        .expect("stake call execution failed");

    assert!(stake_result.success, "Staking 10,000 tokens failed");
    println!(
        "  ✅ Staking 10,000 Tokens Successful (Gas: {})",
        stake_result.gas_used
    );
    ...
    let withdraw_result = executor
        ...
        .await
        .expect("withdrawStaked call execution failed");

    assert!(
        withdraw_result.success,
        "Withdrawing 4,000 staked tokens failed"
    );
    println!(
        "  ✅ Withdrew 4,000 Staked Tokens Successful (Gas: {})",
        withdraw_result.gas_used
    );
