use prometheus::{histogram_opts, opts, Counter, Encoder, Gauge, Histogram, Registry, TextEncoder};
use std::sync::Arc;
...
impl NornMetrics {
    ...
    pub fn new(_config: &MetricsConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // Blockchain metrics
        let block_height =
            Gauge::with_opts(opts!("norn_block_height", "Current blockchain height"))?;
        ...
        // Network metrics
        let connected_peers =
            Gauge::with_opts(opts!("norn_connected_peers", "Number of connected peers"))?;
        ...
        // Error metrics
        let errors_total =
            Counter::with_opts(opts!("norn_errors_total", "Total number of errors"))?;
        ...
        let rpc_errors_total =
            Counter::with_opts(opts!("norn_rpc_errors_total", "Total number of RPC errors"))?;
    ...
    pub fn record_network_activity(
        ...
        self.messages_received_total
            .inc_by(messages_received as f64);
        debug!(
            "Recorded network activity: {} peers, {} sent, {} received",
    ...
    pub fn record_rpc_request(&self, method: &str, response_time: Duration, success: bool) {
        ...
        debug!(
            "Recorded RPC request: {} in {:?} (success: {})",
...
impl MetricsCollector {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!(
            "Metrics collection started with interval: {}s",
